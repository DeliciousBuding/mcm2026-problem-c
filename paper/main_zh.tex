...
\begin{takeawaybox}
\textbf{方法论一致性说明.} 主流程采用 MaxEnt 可行区域采样（Dirichlet 提案 + 约束筛选），LP/MILP 仅用于局部验证。稳定性按“同机制扰动前后”计算以保证可比性。DAWS 采用基于 $U_t$ 的三档风控协议（P75/P90 分位阈值 + 决赛红档覆盖），judge-save 曲线使用校准的 $\beta=4.0$ 作为示意。
\end{takeawaybox}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Tier 1（绿）.} $R_t<Q_{0.75}$ \;\;$\Rightarrow$ 常规 50/50（评委/观众各半）。
\item \textbf{Tier 2（黄）.} $Q_{0.75}\le R_t<Q_{0.90}$ \;\;$\Rightarrow$ 启动 judge-save（bottom-two + save）。
...
\caption{DAWS 触发与档位（上：$U_t$ 与 P75/P90 分位阈值；下：对应的档位阶梯，绿/黄/红）。}
...
\end{equation}
在黄档风险周，我们将评委视为更果断的把关者，设定 $\beta=4.0$，以体现其纠正明显人气偏差的意图。
\begin{figure}[H]
\centering
\includegraphics[width=0.90\linewidth]{figures/fig_judgesave_curve.pdf}
\caption{Judge-save 决策曲线（横轴评委分差，纵轴淘汰概率；示意曲线使用 $\beta=4.0$，分差越大则低分选手被淘汰概率越高）。}
\end{figure}
...
我们对 DAWS 的触发分位阈值与基准权重进行网格扫描，以展示设计空间。扫描结果显示较高的触发阈值（如 $P_{90}\approx0.95$）在复合指标上更优，但我们主动选择更“积极”的阈值（$P_{90}=0.75$），确保风控协议在足够多的周次介入，从而提升可执行性与可见度。
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_param_optimization.pdf}
\caption{DAWS 参数空间扫描：热力图为综合效用分数（公正性 + 能动性 + 稳定性）。}

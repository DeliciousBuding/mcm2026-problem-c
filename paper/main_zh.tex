...
\end{tikzpicture}
\end{center}
\textbf{建议.} 采用分段阈值化的 DAWS 权重 $\alpha_t$，并公开 bottom-two 与 judge-save 判定标准。
\end{minipage}
...
\item DAWS 提升能动性与评委一致性，但稳定性略有代价（权衡方案）。
\end{itemize}
...
\textbf{建议.}
\begin{enumerate}[leftmargin=2em]
\item 公布 DAWS 方案并基于不确定性指数 $U_t$ 的分段阈值规则调整 $\alpha_t$。
...
\begin{takeawaybox}
\textbf{方法论一致性说明.} 主流程采用 MaxEnt 可行区域采样（Dirichlet 提案 + 约束筛选），LP/MILP 仅用于局部验证。稳定性按“同机制扰动前后”计算以保证可比性。DAWS 采用基于 $U_t$ 分位的分段阈值权重，judge-save 曲线使用校准的 $\beta=1.8$ 作为示意。
\end{takeawaybox}
...
\takeaway{DAWS 采用分段阈值权重：常规周保持 50/50，高不确定周提高评委权重（透明可执行）。}
\begin{equation}
\alpha_t=
\begin{cases}
\alpha_0,& U_t\le q_{90},\\
\alpha_1,& q_{90}<U_t\le q_{97},\\
\alpha_2,& U_t>q_{97},
\end{cases}
\end{equation}
其中 $q_{90},q_{97}$ 为 $U_t$ 的分位阈值（仅在最不确定的周触发更高权重）。
图~\ref{fig:daws-trigger} 展示阈值触发与权重档位的对应关系，强调规则可解释与可执行。
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig_daws_trigger.pdf}
\caption{DAWS 触发与权重档位（上：$U_t$ 与公开阈值；下：对应的评委权重档位阶梯，常规/争议/极端周）。\textit{注：阈值为超参数，设计时权衡响应速度与虚警率。}}
...
\begin{equation}
|c_{i,t}-c_{i,t-1}|\le |\alpha_t-\alpha_{t-1}||j_{i,t}-v_{i,t}|+(1-\alpha_t)\|\mathbf v_t-\mathbf v_{t-1}\|+\alpha_t\|\mathbf j_t-\mathbf j_{t-1}\|.
\end{equation}
\end{proposition}
...
\keyoutput{DAWS 方案、性质与校准的 judge-save 行为刻画。}

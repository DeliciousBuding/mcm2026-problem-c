...
\end{tikzpicture}
\end{center}
\textbf{建议.} 采用 DAWS 三档风控协议（红/黄/绿），并公开 bottom-two 与 judge-save 判定标准。
\end{minipage}
...
\textbf{方案.} 我们提出 DAWS 三档风控协议，由不确定性指数触发：绿档为常规 50/50；黄档启动 judge-save；红档（决赛周）完全观众投票。该方案规则公开、易于解释且可直接执行。
...
\captionof{figure}{机制权衡（雷达图）（轴为能动性/评委一致性/稳定，数值为周度加权平均）。}
\end{minipage}\hfill
\begin{minipage}[t]{0.49\linewidth}
\centering
\includegraphics[width=\linewidth]{figures/fig_mechanism_compare.pdf}
\captionof{figure}{机制对比（数值）（柱状对比三指标，便于直观比较）。}
\end{minipage}
\clearpage
\tableofcontents
\clearpage
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{引言与路线图}
\takeaway{我们将 DWTS 视为“审计—压力测试—风控—可视化监控”的完整闭环。}
我们观测到每周评委分数与淘汰结果，但粉丝投票是潜变量。目标不是猜测唯一投票值，而是给出与规则一致的完整可行集合，并将不确定性传播到反事实机制评估与规则设计中。
我们的流程强调可执行性：先审计可行粉丝票集合，再用合成数据压力测试，随后部署 DAWS 三档风控协议，并通过仪表盘输出可执行决策口径。
...
\begin{takeawaybox}
\textbf{方法论一致性说明.} 主流程采用 MaxEnt 可行区域采样（Dirichlet 提案 + 约束筛选），LP/MILP 仅用于局部验证。稳定性按“同机制扰动前后”计算以保证可比性。DAWS 采用基于 $U_t$ 的三档风控协议（P80/P90 分位阈值 + 决赛红档覆盖），judge-save 曲线使用校准的 $\beta=2.0$ 作为示意。
\end{takeawaybox}
...
\takeaway{DAWS 是三档风控协议：红/黄/绿，对应清晰可执行的动作。}
我们定义风险指数 $R_t=U_t$（周度 HDI 宽度），并采用离散规则：
\begin{itemize}[leftmargin=2em]
\item \textbf{Tier 1（绿）.} $R_t<Q_{0.80}$ \;\;$\Rightarrow$ 常规 50/50（评委/观众各半）。
\item \textbf{Tier 2（黄）.} $Q_{0.80}\le R_t<Q_{0.90}$ \;\;$\Rightarrow$ 启动 judge-save（bottom-two + save）。
\item \textbf{Tier 3（红）.} $R_t\ge Q_{0.90}$ 或决赛周 \;\;$\Rightarrow$ 100\% 观众投票。
\end{itemize}
阈值采用分位数并可公开，强调规则透明与可执行。
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig_daws_trigger.pdf}
\caption{DAWS 触发与档位（上：$U_t$ 与 P80/P90 分位阈值；下：对应的档位阶梯，绿/黄/红）。}
...
\end{equation}
在黄档风险周，我们将评委视为纠偏把关者，设定 $\beta=2.0$，以体现对明显人气偏差的校正。
\begin{figure}[H]
\centering
\includegraphics[width=0.90\linewidth]{figures/fig_judgesave_curve.pdf}
\caption{Judge-save 决策曲线（横轴评委分差，纵轴淘汰概率；示意曲线使用 $\beta=2.0$，分差越大则低分选手被淘汰概率越高）。}
\end{figure}

\keyoutput{DAWS 档位协议与校准的 judge-save 行为刻画。}
...
\item \textbf{落地建议：} 公布 DAWS 档位规则与 judge-save 规则以提升透明度。
\end{itemize}
...
\subsection{DAWS 参数扫描}
我们对 DAWS 的触发分位阈值、基准权重与 $\beta$ 进行网格扫描，并施加“活跃度约束”（触发率 20\%--35\%）以避免机制过度被动。约束优化结果选择 $P_{90}=0.80$、$P_{97}=0.90$、$\alpha_{\text{base}}=0.60$、$\beta=2.0$，对应触发率 20.3\%，在公正性与能动性之间取得均衡。
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_param_optimization.pdf}
\caption{DAWS 参数空间扫描：热力图为综合效用分数（公正性 + 能动性 + 稳定性，取不同 $\beta$ 的最大值）。}
\label{fig:param-optimization}
\end{figure}

...
\begin{takeawaybox}
\textbf{方法论一致性说明.} 主流程采用 MaxEnt 可行区域采样（Dirichlet 提案 + 约束筛选），LP/MILP 仅用于局部验证。稳定性按“同机制扰动前后”计算以保证可比性。DAWS 采用基于 $U_t$ 的三档风控协议（固定阈值 + 决赛红档覆盖），judge-save 曲线使用校准的 $\beta=1.8$ 作为示意。
\end{takeawaybox}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Tier 1（绿）.} $R_t<0.4$ \;\;$\Rightarrow$ 常规 50/50（评委/观众各半）。
\item \textbf{Tier 2（黄）.} $R_t\ge 0.4$ \;\;$\Rightarrow$ 启动 judge-save（bottom-two + save）。
\item \textbf{Tier 3（红）.} 决赛周 \;\;$\Rightarrow$ 100\% 观众投票。
\end{itemize}
阈值固定且可公开，强调规则透明与可执行。
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig_daws_trigger.pdf}
\caption{DAWS 触发与档位（上：$U_t$ 与风险阈值；下：对应的档位阶梯，绿/黄/红）。}

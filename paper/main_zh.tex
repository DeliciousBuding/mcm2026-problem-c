...
\begin{takeawaybox}
\textbf{方法论一致性说明.} 主流程采用 MaxEnt 可行区域采样（Dirichlet 提案 + 约束筛选），LP/MILP 仅用于局部验证。稳定性按“同机制扰动前后”计算以保证可比性。DAWS 采用基于 $U_t$ 的三档风控协议（P75/P90 分位阈值 + 决赛红档覆盖），judge-save 曲线使用校准的 $\beta=3.5$ 作为示意。
\end{takeawaybox}
...
\end{equation}
在黄档风险周，我们将评委视为更果断的把关者，设定 $\beta=3.5$，以体现其纠正明显人气偏差的意图。
\begin{figure}[H]
\centering
\includegraphics[width=0.90\linewidth]{figures/fig_judgesave_curve.pdf}
\caption{Judge-save 决策曲线（横轴评委分差，纵轴淘汰概率；示意曲线使用 $\beta=3.5$，分差越大则低分选手被淘汰概率越高）。}
\end{figure}

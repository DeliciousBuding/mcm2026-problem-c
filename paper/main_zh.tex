...
\begin{takeawaybox}
\textbf{方法论一致性说明.} 主流程采用 MaxEnt 可行区域采样（Dirichlet 提案 + 约束筛选），LP/MILP 仅用于局部验证。稳定性按“同机制扰动前后”计算以保证可比性。DAWS 采用“亲公正性安全网”设计，公开分位阈值（P80/P90），Green 档基准权重 $\alpha=0.65$，并在决赛周强制红档；judge-save 曲线使用校准的 $\beta=4.0$ 作为示意。
\end{takeawaybox}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Tier 1（绿）.} $R_t<Q_{0.80}$ \;\;$\Rightarrow$ 65/35（评委/观众，更强调专业性）。
...
\end{itemize}
DAWS 被定义为“亲公正性安全网”：常规周强化专业性（$\alpha=0.65$），争议周启动更果断的评委拯救（$\beta=4.0$），仅在决赛周完全交由观众投票。阈值可公开，强调规则透明与可执行。
\begin{figure}[H]
\centering
...
\end{equation}
在黄档风险周，我们将评委视为更果断的把关者，设定 $\beta=4.0$，以体现对明显人气偏差的强纠偏。
\begin{figure}[H]
\centering

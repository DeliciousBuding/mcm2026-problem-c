...
\caption{争议人物的反事实淘汰风险时间线（纵轴为每周被淘汰概率，四条线对应 percent/rank/judge-save/DAWS；用于回答“规则改变是否改变淘汰风险”）。}
...
\caption{观众参与度与稳定性的 Pareto 权衡，颜色表示评委一致性（曲线为 $\alpha$ 扫描，星标为 DAWS）。}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\linewidth}
\centering
...
\caption{DAWS 触发与权重档位（上：$U_t$ 与公开阈值；下：对应的评委权重档位阶梯，常规/争议/极端周）。}

...
\captionof{figure}{机制权衡（雷达图）（轴为公平/主权/稳定，数值为周度加权平均）。}
\end{minipage}\hfill
...
\begin{algorithm}[H]
\caption{Percent 周度多面体审计（LP 约束与上下界，输出可行区间与松弛度）。}
\begin{algorithmic}[1]
...
\begin{algorithm}[H]
\caption{Rank 可行序列到 share 采样（MILP 枚举可行序列后再采样）。}
\begin{algorithmic}[1]
\Require Rank 规则周数据
...
\centering
\includegraphics[width=\linewidth]{figures/fig_mechanism_compare.pdf}
\caption{机制对比（数值）（三机制在公平/主权/稳定上的对比）。}
\end{subfigure}
\end{figure}

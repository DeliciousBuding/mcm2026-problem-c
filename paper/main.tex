...
\end{tikzpicture}
\end{center}
\textbf{Recommendation.} Adopt DAWS with thresholded $\alpha_t$ tiers and publish bottom-two plus judge-save criteria.
\end{minipage}
...
\item DAWS improves agency, integrity, and stability simultaneously when weights are thresholded by uncertainty.
\end{itemize}
...
\item \textbf{Drivers differ for judges vs fans.} Mixed-effects models show pro-dancer influence is stronger for fans, while judges are less sensitive to pro-identity heterogeneity.
\end{enumerate}

\textbf{Recommendations.}
\begin{enumerate}[leftmargin=2em]
\item Publish a DAWS schedule $\alpha_t$ with thresholded tiers based on an uncertainty index $U_t$.
...
\takeaway{We model DWTS as an audit-and-design problem: characterize feasible fan votes, quantify uncertainty, and propose a rule with improved trade-offs.}
...
\takeaway{DAWS uses thresholded weights tied to uncertainty, making the rule transparent and executable.}
We define
\begin{equation}
\alpha_t=
\begin{cases}
...
\end{cases}
\end{equation}
where $q_{90},q_{97}$ are quantiles of $U_t$ and $(\alpha_0,\alpha_1,\alpha_2)$ are fixed public tiers.
\begin{figure}[H]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig_daws_trigger.pdf}
\caption{DAWS trigger schedule: weekly uncertainty $U_t$ with public thresholds and the resulting tiered judge weights.}
\end{figure}
...
With tiered $\alpha_t$, the score change satisfies
\begin{equation}
...
\caption{Judges prefer higher score within the bottom two; the curve uses calibrated $\beta=1.8$ to illustrate sharpness.}
\end{figure}

\keyoutput{DAWS schedule, properties, and calibrated judge-save behavior.}
...
We provide a complete audit of feasible fan votes, show that rank rules create measurable democratic deficit, and propose DAWS to balance agency, integrity, and stability. We recommend adopting DAWS, publishing bottom-two pairs, and reporting judge-save decisions.
\begin{itemize}[leftmargin=2em]
\item \textbf{Decision-ready summary:} Uncertainty is concentrated in a small set of weeks; most weeks are identifiable.
\item \textbf{Mechanism impact:} Rank aggregation increases flips; DAWS improves stability without sacrificing agency or integrity.

...
Weeks with fewer than $N_{\text{strict,min}}$ strict feasible samples are flagged as \emph{Audit-Weak} and excluded from aggregate metrics (they appear as marked points only). \textbf{The goal is not to guarantee all weeks pass}, but to ensure the exclusion ratio $r_{\text{excl}} < 20\%$. Worst-case weeks (with accept rates near $0.008$) may be excluded; this is expected and disclosed. If $r_{\text{excl}} \ge 20\%$, all season-level conclusions are downgraded to ``exploratory.''
...
All parameters below are fixed as \emph{non-adjustable policy}; they are not tuned based on results.
...
\midrule
$\varepsilon_{\text{sum}}$ & $10^{-9}$ & Simplex sum tolerance \\
$\varepsilon_{\text{ord}}$ & $10^{-6}$ & Elimination ordering tolerance \\
$N_{\text{strict,min}}$ & 500 & Minimum strict feasible samples per week \\
$q_{\text{gate}}$ & 0.10 & Quantile for budget calculation (fixed) \\
$N_{\text{proposals}}$ & 8000 & Default proposal count (main results) \\
$N_{\text{proposals,fast}}$ & 2000 & Fast mode for smoke tests only \\
$r_{\text{excl,max}}$ & 20\% & Downgrade trigger threshold \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Strict checks}: Simplex + elimination constraints; rank/percent rule checks are diagnostic only.
\item \textbf{Excluded weeks}: Flagged as Audit-Weak, shown as marked points, not used in aggregate statistics.
\item \textbf{Downgrade rule}: If excluded ratio $\ge 20\%$, season-level conclusions become ``exploratory.''
\item \textbf{Worst-case disclosure}: Weeks with accept rate $\approx 0.008$ may be excluded; this is expected and does not invalidate the audit.
\end{itemize}

...
\caption{Judges prefer higher score within the bottom two; the curve uses calibrated $\beta=4.0$ to illustrate sharper decisions in Yellow-tier weeks.}
\end{figure}
...
We scan the DAWS trigger quantiles and base weight to map the design space. The sweep suggests a theoretical optimum at higher trigger thresholds (e.g., $P_{90}\approx0.95$), but we deliberately choose a more active setting ($P_{90}=0.75$) so the risk-control protocol intervenes in a meaningful share of weeks. This choice favors operational visibility over marginal score gains.
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_param_optimization.pdf}
\caption{DAWS design space exploration. The heatmap reports the composite utility score (integrity + agency + stability) across trigger quantiles and base weights.}

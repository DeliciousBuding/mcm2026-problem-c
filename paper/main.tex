...
\begin{itemize}[leftmargin=2em]
\item \textbf{Tier 1 (Green).} $R_t<Q_{0.80}$ \;\;$\Rightarrow$ standard 50/50 percent rule.
\item \textbf{Tier 2 (Yellow).} $Q_{0.80}\le R_t<Q_{0.90}$ \;\;$\Rightarrow$ activate judge-save (bottom-two + save).
...
\caption{DAWS trigger schedule: weekly uncertainty $U_t$, with dashed lines marking the P80/P90 thresholds and the resulting tier (Green/Yellow/Red).}
...
\end{equation}
In the Yellow risk tier, we treat judges as corrective gatekeepers and set $\beta=2.0$ to reflect a calibrated response against popularity bias.

% Judge-save 决策曲线
\begin{figure}[H]
\centering
\includegraphics[width=0.90\linewidth]{figures/fig_judgesave_curve.pdf}
\caption{Judges prefer higher score within the bottom two; the curve uses calibrated $\beta=2.0$ to illustrate the Yellow-tier decision sensitivity.}
\end{figure}
...
\subsection{DAWS Parameter Scan}
We scan the DAWS trigger quantiles and base weight to map the design space, and enforce an activation-rate constraint (20--35\%) to avoid a passive policy. The constrained optimum selects $P_{90}=0.80$, $P_{97}=0.90$, $\alpha_{\text{base}}=0.60$, and $\beta=2.0$, yielding a 20.3\% activation rate while balancing integrity and agency. This choice favors operational visibility over marginal score gains.
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_param_optimization.pdf}
\caption{DAWS design space exploration. The heatmap reports the composite utility score (integrity + agency + stability) across trigger quantiles and base weights (max over $\beta$).}

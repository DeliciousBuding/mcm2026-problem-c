...
\begin{figure}[H]
\centering
\placeholderfig{2.1in}{Ridgeline posterior for controversy cases across weeks.}
...
\subsection{Predictive Add-on: XGBoost}
\takeaway{We include a predictive model as a robustness check, not as the main driver analysis.}
We train an XGBoost classifier to predict elimination using forward-chaining validation and report AUC as a sanity check on covariate relevance.
\begin{figure}[H]
\centering
\placeholderfig{1.8in}{Forward-chaining AUC over seasons (XGBoost).}

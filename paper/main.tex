...
\end{tikzpicture}
\end{center}
\textbf{Recommendation.} Adopt DAWS as a cascading protocol (finale override, conflict-triggered judge-save with a score-gap gate, otherwise percent) and publish bottom-two plus judge-save criteria.
\end{minipage}
...
\textbf{Solution.} We propose DAWS as a cascading protocol: a finale override (audience-only), a conflict trigger $A_t$ (judge-save with a score-gap gate), and a default Percent rule otherwise. The uncertainty signal $V_t$ is used for disclosure and audit budget only, not for intervention. The protocol is public, explainable, and easy to execute on-air.
...
When the feasible sampler yields too few accepted proposals ($n_{\text{accept}}<200$) or requires fallback, we flag the week as \textit{Audit-Weak}. In these cases, $V_t$ is treated as a disclosure/monitoring signal rather than a decision trigger, and we surface the flag in audit metadata for transparency.
...
\item \textbf{Conflict (A=1).} If they disagree, activate judge-save \emph{only when the judge-share gap exceeds a fixed threshold} ($\Delta$); otherwise follow Percent to preserve agency.
...
We also apply a fixed evidence gate: judge-save is used only when the judge-share gap exceeds $\Delta=0.05$; otherwise DAWS follows Percent to preserve agency.

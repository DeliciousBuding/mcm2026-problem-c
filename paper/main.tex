...
\begin{takeawaybox}
\textbf{Methodology Alignment Box.} Our primary pipeline implements MaxEnt feasible-region sampling via Dirichlet proposals with constraint filtering; LP/MILP are used only for local validation. Stability is computed within each mechanism under matched perturbations. DAWS uses a public three-tier risk protocol based on $U_t$ with a fixed threshold and a final-week Red override; the judge-save curve uses a calibrated $\beta=1.8$ for illustration.
\end{takeawaybox}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Tier 1 (Green).} $R_t<0.4$ \;\;$\Rightarrow$ standard 50/50 percent rule.
\item \textbf{Tier 2 (Yellow).} $R_t\ge 0.4$ \;\;$\Rightarrow$ activate judge-save (bottom-two + save).
\item \textbf{Tier 3 (Red).} Final week \;\;$\Rightarrow$ 100\% audience vote.
\end{itemize}
The threshold is fixed and publishable, making the policy transparent and executable.
...
\caption{DAWS trigger schedule: weekly uncertainty $U_t$ and the resulting tier (Green/Yellow/Red).}

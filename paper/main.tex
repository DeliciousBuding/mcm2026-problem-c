...
\begin{takeawaybox}
\textbf{Methodology Alignment Box.} Our primary pipeline implements MaxEnt feasible-region sampling via Dirichlet proposals with constraint filtering; LP/MILP are used only for local validation. Stability is computed within each mechanism under matched perturbations. DAWS is a pro-integrity safety net with publishable quantile thresholds (P80/P90), a Green-tier base weight $\alpha=0.65$, and a final-week Red override; the judge-save curve uses a calibrated $\beta=4.0$ for illustration.
\end{takeawaybox}
...
\begin{itemize}[leftmargin=2em]
\item \textbf{Tier 1 (Green).} $R_t<Q_{0.80}$ \;\;$\Rightarrow$ pro-integrity 65/35 split (judges/fans).
...
\end{itemize}
DAWS is a \textit{Pro-Integrity Safety Net}: in standard weeks it emphasizes professional judgment ($\alpha=0.65$), in high-risk weeks it activates judge-save with a decisive $\beta=4.0$, and only in the finale does it fully yield to the audience. Thresholds are publishable and operational.
...
\end{equation}
In the Yellow risk tier, we treat judges as decisive gatekeepers and set $\beta=4.0$ to reflect a strong corrective response against popularity bias.

% Judge-save 决策曲线
\begin{figure}[H]
\centering
\includegraphics[width=0.90\linewidth]{figures/fig_judgesave_curve.pdf}
\caption{Judges prefer higher score within the bottom two; the curve uses calibrated $\beta=4.0$ to illustrate the Yellow-tier decision sensitivity.}
\end{figure}
...
% DAWS parameter scan removed for final override (fixed pro-integrity configuration).

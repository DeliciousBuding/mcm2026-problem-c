...
\item DAWS increases agency and judge integrity but trades off some stability under thresholded weights.
\end{itemize}
...
We train a gradient-boosted decision tree (GBDT) classifier to predict elimination using forward-chaining validation and report AUC as a sanity check on covariate relevance. Predictive performance is stable and supports the selected covariates; see Appendix~\ref{app:auc} for the calibration curve.
...
Figure~\ref{fig:daws-trigger} shows the tiered trigger schedule implied by these thresholds.
\begin{figure}[H]
\centering

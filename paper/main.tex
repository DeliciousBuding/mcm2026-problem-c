...
\end{tikzpicture}
\end{center}
\textbf{Recommendation.} Adopt DAWS as a cascading protocol (finale override, conflict-triggered judge-save, otherwise percent) and publish bottom-two plus judge-save criteria.
\end{minipage}
...
\captionof{figure}{Mechanism comparison (all weeks).}
\end{minipage}
\clearpage
\tableofcontents
\clearpage
...
\takeaway{We model DWTS as an audit-and-design problem: audit feasible votes, stress-test uncertainty, deploy a conflict-triggered protocol with disclosure tiers, and present a producer dashboard.}
...
Our workflow is intentionally operational: audit the existing system (feasible-region analysis), stress-test with synthetic validation, deploy DAWS as a cascading conflict-triggered protocol with disclosure tiers, and expose decisions through a dashboard that producers can execute and communicate on-air.
...
\end{equation}
\textit{Note: A formal $\Delta$ min-gap can be introduced for theoretical analysis, but our implementation \textbf{does not enforce any min-gap}; only the bottom-$k$ ordering constraint is used ($\max(\text{score}_E)\le\min(\text{score}_S)+\epsilon$, $\epsilon=10^{-6}$). See Appendix~\ref{app:lp-scope}.}
...
\takeaway{We fix all tolerance parameters and sampling budgets as non-adjustable policy to avoid post-hoc tuning.}
...
\takeaway{DAWS is a conflict-triggered protocol that patches rule disagreement.}
...
\caption{Judges prefer higher score within the bottom two; the curve uses calibrated $\beta=6.0$ to illustrate conflict-week decision sensitivity.}
\end{figure}

\keyoutput{Conflict-triggered DAWS protocol and calibrated judge-save behavior.}
...
\item \textbf{Methodological limitations:} Double-elimination weeks have smaller feasible regions (acceptance rate $R \approx 30\%$ of single-elimination), consistent with combinatorial expectation (Appendix~\ref{app:double-elim}); LP/MILP is used for diagnostics only (Appendix~\ref{app:lp-scope}).
\end{itemize}
...
\section{Double-Elimination Week Feasibility Verification}\label{app:double-elim}
...
\paragraph{Pre-registered Smoke Test (R-ratio)}
We pre-registered $R = \text{median}(\text{accept\_rate}_{k=2}) / \text{median}(\text{accept\_rate}_{k=1})$ with expected range $[0.5, 2.0]$. Observed $R = 0.28$, \textbf{FAIL}. This result is retained (not adjusted) because $R < 0.5$ is mathematically expected: double-elimination constraints are stricter, yielding smaller feasible regions.

\paragraph{Theoretical Baseline Alignment}
From combinatorics: $R_{\text{baseline}} = 2/(n-1)$ where $n$ is the number of active contestants. For median $n=8$, $R_{\text{baseline}} = 0.286$. Observed $R / R_{\text{baseline}} \approx 1$, confirming the ratio matches theoretical expectation rather than an implementation bug.

\paragraph{Structural Recomputation (Primary Evidence)}
To avoid circular self-verification, we explicitly recompute combined scores and residuals ($\text{residual} = \max(\text{score}_E) - \min(\text{score}_S)$) on accepted samples, \textbf{without calling} \texttt{strict\_feasible\_mask}. Results:
\begin{itemize}[leftmargin=2em]
\item Double-elimination weeks: total=31, valid=15, checked=15, min\_check\_rate=1.0 \checkmark
\item Single-elimination weeks: total=232, valid=225, checked=225, min\_check\_rate=1.0 \checkmark
\end{itemize}
All accepted samples pass independent recomputation, verifying constraint encoding correctness.

\section{LP/MILP Scope Declaration}\label{app:lp-scope}
This appendix clarifies the role of LP/MILP in this paper: \textbf{diagnostic tool only}, not part of the main inference pipeline.

\paragraph{Main Model Implementation}
MaxEnt Dirichlet sampling + \texttt{strict\_feasible\_mask()} constraint filtering. Posterior is a sample-based approximation, not an LP optimization solution.

\paragraph{LP/MILP Usage (Diagnostic)}
\begin{itemize}[leftmargin=2em]
\item \texttt{slack\_cache}: audit metadata (constraint tension indicator)
\item Bounds visualization: reference only, not primary conclusions
\end{itemize}

\paragraph{LP/MILP NOT Used For}
Sampling filtering, feasibility checking, posterior estimation, or mechanism metric computation. Current implementation is a stub (returns fixed slack=0.001).

See output file \texttt{audit\_lp\_milp\_scope.json} for machine-verifiable declaration.
...
\section{Audit Parameter Specification Table}\label{app:audit-params}
All parameters below are fixed \emph{a priori}; they are not tuned based on results.

\begin{table}[H]
\centering
\caption{Fixed audit parameters (Block 5 specification).}
...
\item \textbf{Audit rules} define feasibility and output eligibility; they are not optimization targets.
\item \textbf{Compute budget} affects Monte Carlo precision but is not a model assumption.
\item \textbf{Reporting rule}: The 20\% threshold triggers downgrade, not parameter re-tuning.
\item \textbf{Ties allowed}: Eliminated contestants may tie with survivors ($\max E = \min S$).
\item \textbf{Excluded weeks}: Flagged as Audit-Weak, shown as marked points, not in aggregates.
\item \textbf{Worst-case disclosure}: Weeks with accept rate $\approx 0.008$ may be excluded; this is expected.
\item \textbf{Diagnostic score source}: Percent-rule diagnostics use \texttt{judge\_share} as an audit proxy for the combined percent score used in our elimination model.
\item \textbf{Rank threshold}: $\varepsilon_{\text{rank}}=0$ means eliminated contestants' rank must be $\le$ survivors' rank (ties allowed).
\end{itemize}

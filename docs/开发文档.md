...
## 10. Git 工作规范与版本管理
- 禁止直接在 `main` 分支提交。
- 每个 Block/明确任务新建独立分支（如 `block0-method-alignment`、`block1-core-evidence`、`block2-captions-refs`）。
- 一个分支只做该 Block 的内容，不跨 Block 修改。
- 分支内可多次提交，提交信息必须中文且清楚描述“做了什么”。
- 每次提交后必须同步推送远程。
- 只有在通过验收口径并获得确认后，才能合并回 `main`；合并后提示打 tag（不在此处自动执行）。
- 如发现任务不清楚、需要跨 Block 修改或影响已完成 Block，必须先停下并确认。
- 版本号维护在本文件“版本信息”中，作为统一版本入口。

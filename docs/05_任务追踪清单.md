...
### Block 5 核心任务（执行顺序）
- [ ] **Hard-1：采样兜底不可行样本**
  - **修实现（优先）**：兜底样本必须满足 elimination 约束；禁止再用 `proposals[:10]` 直接填充。
  - **降级叙事（替代）**：将 fallback 周标记为 *Audit-Weak* 并排除出后验/指标，仅保留为“审计失败告警”。
  - **验收**：后验/HDI/机制指标仅使用“可行样本”；fallback 周不污染统计。
  - **产物**：`outputs/audit_week_meta.csv` 新增 `strict_feasible_flag` 与 `used_fallback`。

- [ ] **Hard-2：LP/MILP/Slack 诊断为空**
  - **修实现（优先）**：实现 `lp_bounds_and_slack` 的最小可用版本（至少输出可行性/松弛度量）。
  - **降级叙事（替代）**：正文删除“LP/MILP 验证”为主线，仅保留“概念上可做、本文未实现”的注释。
  - **验收**：正文/附录/代码一致，不再出现“已完成 LP/MILP 验证”的暗示。

- [ ] **Hard-3：双淘汰规则编码错误**
  - **修实现（必须）**：percent/rank 的 strict 检查改为“落在 bottom-k”，而非“全部等于全场最小”。
  - **验收**：双淘汰周的可行域不再过度收缩；accept_rate 与 HDI 不再异常偏低。

- [ ] **Hard-4：Temporal smoothness 仅停留在文本**
  - **修实现（优先）**：实现最小跨周平滑（例如对后验均值/HDI 做时序正则或后处理）。
  - **降级叙事（替代）**：删除“temporal smoothness 作为主模型”的表述，仅保留“探索性后处理”。
  - **验收**：正文方法与主流程一致，避免“宣称但未实现”。

- [ ] **Hard-5：Rank→share 强加 Δ 间隔**
  - **修实现（优先）**：Δ 仅用于 tie-breaking 或设为 0（不强加最小差值）。
  - **降级叙事（替代）**：删除“强制 Δ”表述，改为“弱序一致性”。

- [ ] **Hard-6：fast vs strict 验证被 fallback 污染**
  - **修实现（优先）**：strict 评估只用严格可行样本；不足则剔除该周或标记为 Audit-Weak。
  - **降级叙事（替代）**：将 fast/strict 结果明确标注为“近似健壮性指示”，不作为审计结论。

- [ ] **Hard-7：β/Δ/阈值主观性**
  - **修实现（优先）**：对 β/Δ 做最小敏感性分析或外部约束（如“分差≥1 ⇒ 95% 纠偏”）。
  - **降级叙事（替代）**：明确为“政策参数/治理强度”，不称“校准估计”。

### Block 5 验收口径
- [ ] 每条硬伤要么实现闭环，要么降级并标注局限。
- [ ] 任何主结论不依赖不可行样本或空实现模块。
- [ ] 论文方法描述与代码行为逐条一致。

### Block 5 产物清单
- [ ] 修订后的 `outputs/audit_week_meta.csv` 与 `summary_metrics`（含 strict/fallback 标签）。
- [ ] 修订后的方法段落（中英文同步）。

...
### Block 5 核心任务（执行顺序）
- [ ] **Hard-0：strict feasible 的可执行定义（必须写死）**
  - **simplex**：v_i >= 0 且 sum(v_i)=1（写死 `eps_sum` 数值，仅用于归一化等式）。
  - **elimination**：对淘汰集 E、存活集 S，严格约束为 max(E) <= min(S) + eps（写死 `eps_ord` 数值，仅用于不等式比较）。
  - **ties 规则**：写死“允许并列淘汰”或“并列视为不满足”；若不允许并列，则用 max(E) <= min(S) - eps_ord。
  - **bottom-k 定义**：写死为“按全体排序的 bottom-k”；percent/rank 在 strict 中的地位必须明确为 (A) 不参与可行性，仅诊断 或 (B) 作为额外必要条件。
...
- [ ] **Hard-3：双淘汰规则编码错误**
  ...
  - **验收**：修复前后对比——双淘汰周的 accept_rate_strict 不再系统性低于单淘汰周（写死“同量级”判定阈值）。
...
- [ ] **缺口B：采样失败后的主结论处理规则（必须写死）**
  ...
  - 若被排除周包含 finale / semi-final（按题面周编号写死）⇒ 对相关结论标注不可用。
...
- [ ] 约束口径表（strict/fast checks、eps/ties、缺周处理规则）。
- [ ] 修订后的方法段落（中英文同步）。

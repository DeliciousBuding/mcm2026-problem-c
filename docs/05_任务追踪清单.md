更新日期：2026-02-02 | 版本：v0.3.5 | 说明：英文为准，中文可能滞后。
...
## 最新进度摘要（2026-02-02）
- **Block 5 Hard-3 结构性复核完成**：预注册 R smoke test FAIL（R=0.2834），但结构性复核 PASS（双淘汰15周+单淘汰225周，min_check_rate=1.0）
- **防循环自证声明已写入**：`recompute_path` + `reuse_guard` 字段
- **覆盖口径完整**：total/valid/excluded/checked 拆分，excluded 周不进入 correctness 结论
- **论文已编译**：main.pdf (32页) + main_zh.pdf (31页)

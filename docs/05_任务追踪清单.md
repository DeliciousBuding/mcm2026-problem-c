...
- [√] **Hard-0：strict feasible 的可执行定义（必须写死）** ✅ 已完成
  ...
  - **诊断定义（写死）**：
    ...
    - `percent_rule_diagnostic = 1{residual <= eps_ord}`
    - `rank_rule_residual = max(judge_rank_E) - min(judge_rank_S)`
    - `rank_rule_diagnostic = 1{residual <= eps_ord}`
  - **完成记录**：eps_sum/eps_ord/ties 规则写死；底层残差诊断字段 `percent_rule_residual`/`percent_rule_diagnostic`/`rank_rule_residual`/`rank_rule_diagnostic` 已输出到 audit_week_meta.csv，且与采样完全解耦（仅依赖原始数据与淘汰集 E/S）。
...
- [ ] **Hard-3：双淘汰规则编码错误**
  ...
  - **验收**：定义 `R = median(accept_rate_strict_double) / median(accept_rate_strict_single)`，要求 `0.5 <= R <= 2.0`。

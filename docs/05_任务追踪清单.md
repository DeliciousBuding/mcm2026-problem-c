...
- [ ] **Hard-0：strict feasible 的可执行定义（必须写死）**
  - **simplex**：v_i >= 0 且 sum(v_i)=1（允许数值容差 eps，写死具体 eps）。
  - **elimination**：对淘汰集 E、存活集 S，严格约束为 max(E) <= min(S) + eps（ties 规则写死）。
  - **rank/percent 约束**：明确是“必要条件”还是“软约束”；若与 elimination 冲突，优先级必须写死。
  - **验收**：strict_feasible_flag 的判定完全由上述 checks 给出。
...
- [ ] **Hard-2：LP/MILP/Slack 诊断为空**
  ...
  - **验收**：正文中任何 LP/MILP/optimization 表述必须与代码调用一致。

- [ ] **Hard-3：双淘汰规则编码错误**
  - **修实现（必须）**：percent/rank 的 strict 检查改为 bottom-k：max(E) <= min(S)+eps。
  - **验收**：双淘汰周可行域不再过度收缩；accept_rate 与 HDI 不再异常偏低。

- [ ] **Hard-4：temporal smoothness 只在文本中存在**
  - **修实现（优先）**：实现最小跨周平滑（若仅后处理，必须明确为后处理）。
  - **降级叙事（替代）**：删除“temporal smoothness 作为主模型”的表述，仅保留“探索性后处理”。
  - **验收**：若仅后处理，正文不得使用 posterior/MaxEnt/temporal prior 描述其为主模型。

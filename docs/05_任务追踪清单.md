...
- [√] **Hard-0：strict feasible 的可执行定义（必须写死）** ✅ 已完成
  ...
  - **诊断输出验收**：percent/rank 作为诊断项必须输出到 `audit_week_meta.csv`（`percent_rule_diagnostic`、`rank_rule_diagnostic`），且不影响 strict。
  - **完成记录**：eps_sum/eps_ord/ties 规则写死；诊断字段 `percent_rule_diagnostic`/`rank_rule_diagnostic` 已输出到 audit_week_meta.csv。
